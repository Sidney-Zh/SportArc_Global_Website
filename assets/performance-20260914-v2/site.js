...
function setupDeviceRailProgress() {
  ...
  const mobileCues = window.matchMedia("(max-width: 720px)");
  const visibleRails = new Set();
  const measurements = [];
  let pendingFrame = 0;
  const requestUpdate = () => {
    if (!mobileCues.matches || pendingFrame) return;
    pendingFrame = window.requestAnimationFrame(() => {
      pendingFrame = 0;
      if (!mobileCues.matches) return;
      const updates = measurements
        .filter(({ rail }) => visibleRails.has(rail))
        .map(({ measure }) => measure());
  ...
  const visibilityObserver = "IntersectionObserver" in window
    ? new IntersectionObserver((entries) => {
        entries.forEach(({ target, isIntersecting }) => {
          if (isIntersecting) visibleRails.add(target);
          else visibleRails.delete(target);
        });
        requestUpdate();
      }, { rootMargin: "160px" })
    : null;
  mobileCues.addEventListener("change", requestUpdate);
  ...
  rails.forEach((rail) => {
    ...
    measurements.push({ rail, measure: () => {
    ...
    if (visibilityObserver) visibilityObserver.observe(rail);
    else visibleRails.add(rail);
...
if (sharedTopbar) {
  ...
  const requestTopbarUpdate = () => {
    ...
    topbarFrame = window.requestAnimationFrame(() => {
      ...
      sharedTopbar.classList.toggle("is-scrolled", scrolled);
    });
  };
  // Read scroll position after the initial layout, including restored pages.
  window.addEventListener("scroll", requestTopbarUpdate, { passive: true });
  window.addEventListener("pageshow", requestTopbarUpdate);
}
