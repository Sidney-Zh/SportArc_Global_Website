(function () {
  ...
  const COPY = {
    en: {
      ...
      description:
        "We use cookies to keep this website working, understand how it is used and improve your experience. Read our",
      necessaryOnly: "Necessary cookies only",
      allowAll: "Accept all cookies",
      settings: "Cookie settings",
      privacy: "Privacy Policy",
      choice: ". You can accept all cookies or use necessary cookies only.",
    ...
    "zh-CN": {
      title: "Cookie 设置",
      description:
        "我们使用 Cookie 保障网站运行、了解使用情况并改进体验。请查看我们的",
    ...
    "zh-TW": {
      title: "Cookie 設定",
      description:
        "我們使用 Cookie 維持網站運作、瞭解使用情況並改善體驗。請查看我們的",
      necessaryOnly: "僅使用必要 Cookie",
      allowAll: "全部允許",
      settings: "Cookie 設定",
      privacy: "隱私權政策",
      choice: "。您可以選擇全部允許，或僅使用必要 Cookie。",
    ...
    de: {
      ...
      description:
        "Wir verwenden Cookies, um die Website zu betreiben, ihre Nutzung zu verstehen und dein Nutzungserlebnis zu verbessern. Mehr dazu in unserer",
      necessaryOnly: "Nur notwendige Cookies",
      allowAll: "Alle Cookies zulassen",
      settings: "Cookie-Einstellungen",
      privacy: "Datenschutzerklärung",
      choice: ". Du kannst alle oder nur notwendige Cookies zulassen.",
    ...
    fr: {
      ...
      description:
        "Nous utilisons des cookies pour faire fonctionner le site, comprendre son utilisation et améliorer votre expérience. Consultez notre",
      necessaryOnly: "Cookies nécessaires uniquement",
      allowAll: "Accepter tous les cookies",
      settings: "Paramètres des cookies",
      privacy: "politique de confidentialité",
      choice: ". Vous pouvez accepter tous les cookies ou uniquement les cookies nécessaires.",
    ...
    ja: {
      title: "Cookie 設定",
      description:
        "サイトの運営、利用状況の把握、使いやすさの向上のために Cookie を使用します。詳しくは",
      necessaryOnly: "必要な Cookie のみ",
      allowAll: "すべての Cookie を許可",
      settings: "Cookie 設定",
      privacy: "プライバシーポリシー",
      choice: "をご覧ください。すべての Cookie を許可するか、必要な Cookie のみにするかを選べます。",
      detailTitle: "Cookie 設定",
      detailDescription:
    ...
    ko: {
      title: "쿠키 설정",
      description:
        "웹사이트 운영, 이용 현황 파악 및 이용 경험 개선을 위해 쿠키를 사용합니다. 자세한 내용은",
      necessaryOnly: "필수 쿠키만 사용",
      allowAll: "모든 쿠키 허용",
      settings: "쿠키 설정",
      privacy: "개인정보 처리방침",
      choice: "을 확인해 주세요. 모든 쿠키 또는 필수 쿠키만 허용할 수 있습니다.",
      detailTitle: "쿠키 설정",
      detailDescription:
  ...
  // Yield one rendering opportunity, without delaying analytics by a fixed interval.
  function afterFirstRender(callback) {
    if (document.hidden || document.readyState === "complete" ||
        window.performance?.getEntriesByType?.("paint").length) {
      callback();
      return () => {};
    }
    let frame = 0;
    ...
    frame = window.requestAnimationFrame(() => { task = window.setTimeout(run, 0); });
    return cancel;
  }

  let cancelGtmStart = null;
  function loadGTM(immediate = false) {
    if (gtmRequested || currentConsent !== CONSENT_GRANTED) return;
    if (immediate) { insertGTM(); return; }
    if (!cancelGtmStart) cancelGtmStart = afterFirstRender(insertGTM);
  }

  function insertGTM() {
    if (cancelGtmStart) { cancelGtmStart(); cancelGtmStart = null; }
    if (gtmRequested || currentConsent !== CONSENT_GRANTED) return;
    gtmRequested = true;
      ...
      <div class="analytics-consent__inner">
        <div class="analytics-consent__content">
          <div class="analytics-consent__view" data-consent-summary>
            ...
            <p id="analytics-consent-description">${copy.description} <a href="/privacy/">${copy.privacy}</a>${copy.choice}</p>
          </div>
