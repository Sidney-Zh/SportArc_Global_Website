(function () {
  ...
  // Automatic analytics waits for observed content paint, not the load event.
  function afterFirstRender(callback) {
    const hasFCP = () => window.performance?.getEntriesByType?.("paint")
      .some(entry => entry.name === "first-contentful-paint");
    if (document.hidden || hasFCP() ||
